...
const STORAGE_ROOT = path.resolve(process.cwd(), "storage", "shelby");

// Ensure storage directory exists
async function ensureStorageDir(account: string, namespace: string, voiceId: string) {
...
export async function uploadToShelby(
  ...
  namespace: string, 
  voiceId: string, 
  bundleFiles: Record<string, Buffer>
) {
  try {
    // Parse Shelby URI
 ...
 */
export async function downloadFromShelby(uri: string, filename: string): Promise<Buffer> {
  try {
    // Parse URI
 ...
 */
export async function deleteFromShelby(uri: string, account: string) {
  try {
    // Parse URI
    const match = uri.match(/^shelby:\/\/([^/]+)\/([^/]+)\/(.+)$/);
    if (!match) {
 ...
 */
export async function verifyAccess(uri: string, requesterAccount: string): Promise<boolean> {
  try {
    // Parse URI
    const match = uri.match(/^shelby:\/\/([^/]+)\/([^/]+)\/(.+)$/);
    if (!match) {
